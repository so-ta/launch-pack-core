...
function request(req, res, actionConfig, dataObj) {
  return new Promise(function (resolve, reject) {
    ...
    let apiRequest = generateRequest(req.method, actionConfig["api"], dataObj, reqCookie);
...
function generateRequest(method, apiResource, dataObj, reqCookie) {
  let request = {
    method: method,
    timeout: 30 * 1000,
...
function getRenderData(req, res, workDirectory, actionConfig) {
  ...
  return new Promise(function (resolve, reject) {
    if ('api' in actionConfig) {
      ...
      /***** GET Request *****/
        ...
        return request(req, res, actionConfig, null).then(
          function (resolveObj) {
            resolve(resolveObj);
          }, function (err) {
            reject(err);
