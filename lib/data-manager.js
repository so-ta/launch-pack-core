...
exports.getRenderData = (
  ...
  if ('api' in actionConfig) {
    ...
    const doRequest = (dataObj) => {
      return request(req, res, actionConfig, dataObj, accessLog).then(
        (resolveObj) => {
          resolve(resolveObj);
        }, (reqError) => {
          reject(reqError);
      ...
      case 'POST':
        ...
        if (req.headers['content-type'] === 'application/x-www-form-urlencoded') {
          ...
          form.parse(req, (err, fields, files) => {
            ...
            if (typeof (files) === 'object') {
              ...
              for (let i = 0; i < fileKeys.length; i++) {
                ...
                for (let j = 0; j < files[fileKey].length; j++) {
                  attach_filepaths.push(files[fileKey][j].path);
            ...
            doRequest(dataObj).then(
              (resolveObj) => {
                for (let i = 0; i < attach_filepaths.length; i++) {
                  fs.unlink( attach_filepaths[i], (err) => {
                    if(err){
                      console.error("[LaunchPack] An error occurred to file upload: "+err)
                    } else {
                      console.info(`${moment().format('YYYY-MM-DD HH:mm:ss')} :: File upload done`);
                    }
                  });
                }
                resolve(resolveObj);
