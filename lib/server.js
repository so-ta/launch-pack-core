...
exports.launchServer = (workdic) => {
  ...
  if (fs.existsSync(path.join(workDirectory, 'launchpack.js'))) {
    launchpackCustomFunc = require(path.join(workDirectory, 'launchpack.js')); // eslint-disable-line global-require
    ...
    dataManager
      ...
      .then(
        ...
        (err) => {
          ...
          const specificErrorTemplate = path.join(workDirectory, 'views', 'errors', `${errorStatusCode}.ect`);
