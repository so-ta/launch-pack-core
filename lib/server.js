...
function launchServer(workdic) {
  ...
  const workDirectory = path.join(workdic);

  let launchpackCustomFunc = null;
  if (fs.existsSync(path.join(workDirectory, 'launchpack.js'))) {
    launchpackCustomFunc = require(path.join(workDirectory, 'launchpack.js'));
  ...
  http.createServer((req, res) => {
    ...
    if (global.NODE_ENV === 'local') {
      ...
      if (fs.existsSync(path.join(workDirectory, 'launchpack.js'))) {
        delete require.cache[require.resolve(path.join(workDirectory, 'launchpack.js'))];
        launchpackCustomFunc = require(path.join(workDirectory, 'launchpack.js'));
      }
    ...
    const viewRootDirectory = path.join(workDirectory, 'views');
    ...
    if (reqUrl in resourcesmap) {
      ...
      const hashedFilePath = path.join(workDirectory, 'public', 'hashed', resourcesmap[reqUrl]);
      distribution.distributeByFilePath(hashedFilePath, req, res, accessLog);
      return;
    }

    const filePath = path.join(workDirectory, 'public', reqUrl);
    ...
    const actionString = actionAndParams.actionString;
    const actionParams = actionAndParams.actionParams;
    ...
    for (const pathKey in actionParams) {
    ...
    const is_url_reg = new RegExp('^https?:\/\/');
    ...
    /* 埋め込むデータの探索 */
    data_manager
      .getRenderData(req, res, workDirectory, actionConfig, accessLog)
      .then(
        /* テンプレートを元にレンダリングする */
        (contentString) => {
          ...
          try {
            ...
            if (template !== null) {
              return render.renderTemplate(launchpackConfig, req, res, viewRootDirectory, template, renderObj, launchpackCustomFunc, actionAndParams);
            }
          } catch (e) {
            if (template !== null) {
      ...
      .then(
        ...
        (err) => {
          console.log(err);
          const statusCode = (typeof (err) === 'object' && 'statusCode' in err) ? err.statusCode : 500;
          res.statusCode = statusCode;

          let errorTemplate = 'errors/default.ect';
          const specificErrorTemplate = path.join(workDirectory, 'views', 'errors', `${statusCode}.ect`);
          ...
          render.renderTemplate(launchpackConfig, req, res, viewRootDirectory, errorTemplate, errorRenderObj, launchpackCustomFunc, actionAndParams)
            .then((resultText) => {
              distribution.distributeText(resultText, req, res, accessLog);
            }, (err) => {
