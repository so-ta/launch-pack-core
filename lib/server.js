...
function launchServer(workdic) {
  ...
  http.createServer(function (req, res) {
    ...
    for (let pathKey in actionParams) {
      let param = encodeURIComponent(actionParams[pathKey]);
    ...
    if (actionConfig.hasOwnProperty("api") && launchpackConfig.hasOwnProperty("api_base_url")) {
    ...
    data_manager
      ...
      .then(
        ...
        function (contentString) {
          ...
          try {
            ...
            if ([301, 302, 303, 307].indexOf(renderObj["app_status_code"]) >= 0) {
              if ('redirect' in renderObj) {
                ...
                if ('set-cookie' in res.getHeaders()) {
                  res.setHeader('set-cookie', [res.getHeaders()['set-cookie'], cookie.serialize("lp-flash", JSON.stringify(renderObj["flash"]), {path: '/'})]);
                } else {
                  res.setHeader('set-cookie', [cookie.serialize("lp-flash", JSON.stringify(renderObj["flash"]), {path: '/'})]);
                }
                res.end();
                return;
              }
            ...
            if (template !== null) {
              return render.renderTemplate(launchpackConfig, req, res, viewRootDirectory, template, renderObj);
            }
          } catch (e) {
            if (template !== null) {
      ...
      .then(
        ...
        function (err) {
          ...
          render.renderTemplate(launchpackConfig, req, res, viewRootDirectory, errorTemplate, {
