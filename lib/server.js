...
exports.launchServer = () => {
  ...
  const server = http.createServer((req, res) => {
    ...
    try {
      ...
      distribution.distributePlainText(400, 'Bad Request', req, res);
      return;
    }

    // アクセスログの出力
    console.info(`${moment()
      .format('YYYY-MM-DD HH:mm:ss')} :: ${req.method} ${req.headers.host}${reqUrl.toString()}`);
    ...
    if (global.LaunchPack.DEBUG_MODE) {
      ...
      if (badHealthFile) {
        distribution.distributePlainText(500, `Failed to load ${badHealthFile}`, req, res);
        return;
      }
    }
    ...
    if (reqUrl.pathname === '/health') {
      ...
      distribution.distributePlainText(200, 'ok', req, res);
      return;
    }
    ...
    if (provisionalActionConfig === undefined) {
      distribution.distributePlainText(500, 'ActionConfig Not Found', req, res);
      return;
    }
    ...
    ['api', 'json'].forEach((key) => {
      if (ObjHasProp(actionConfig, key)) {
        Object.keys(actionParams)
          .forEach((pathKey) => {
            const param = actionParams[pathKey];
    ...
    /* 埋め込むデータの探索 */
    dataManager
      .getRenderData(req, res, cwd, actionConfig)
      .then(
        /* テンプレートを元にレンダリングする */
        (contentString) => {
          let renderObj;
          try {
            ...
            if (global.LaunchPack.DEBUG_MODE) {
              renderObj.content_json = contentString;
            }
            ...
            if (ObjHasProp(renderObj, 'app_status_code')
              ...
              if ('redirect' in renderObj) {
                ...
                if (!redirectUrl.match(/^https?:\/\/.*/)) {
                  ...
                  if ('get_params' in renderObj && Object.keys(renderObj.get_params).length > 0) {
                    redirectUrl += `?${querystring.stringify(renderObj.get_params)}`;
                  }
                }
      ...
      .then(
        /* 実際に配信する */
        (resultText) => {
          distribution.distributeText(resultText, req, res);
        },
        (err) => {
          ...
          if (fs.existsSync(specificErrorTemplate)
            && !fs.statSync(specificErrorTemplate)
              .isDirectory()) {
          ...
          render.renderTemplate(
            ...
            .then((resultText) => {
              ...
              if (global.LaunchPack.DEBUG_MODE && renderError instanceof errors.RenderError) {
                distribution.distributeText(renderError.message, req, res);
                return;
              }

              // テキストでエラーを返却する
              distribution.distributePlainText(res.statusCode, `${res.statusCode} Error`, req, res);
            });
        },
      );
  })
  ...
  if ('launchpack_timeout' in config) {
    server.timeout = config.launchpack_timeout * 1000;
  }
